...
def mt_rollout(
    ...
    img = env.render(offscreen=True, camera_name="behindGripper", resolution=img_res)
    if reset_callback:
        reset_callback(env, agent, o)
    if render:
        env.render(**render_kwargs)
    while path_length < max_path_length:
        ...
        next_o, r, done, env_info = env.step(copy.deepcopy(a))
        # Add current image and render next and add to env info
         # breakpoint()
        env_info['img'] = copy.deepcopy(img)
        img = env.render(offscreen=True, camera_name="behindGripper", resolution=img_res)
        env_info['next_img'] = copy.deepcopy(img)
        if render:
            env.render(**render_kwargs)
        observations.append(o)
        rewards.append(r)
